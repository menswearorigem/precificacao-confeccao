...
function Cartao({ evento, onClick }) {
  ...
  return (
    <div
      ref={setNodeRef}
      style={{ ...style, borderLeft: `3px solid ${corDaCategoria(evento.categoria)}`, opacity: evento.podeEditar ? 1 : 0.75, cursor: evento.podeEditar ? undefined : 'pointer' }}
      className={`kanban-cartao${isDragging ? ' arrastando' : ''}`}
      onClick={() => onClick(evento.id)}
      {...listeners}
      {...attributes}
    >
      <div className="kanban-cartao-titulo">{evento.titulo}</div>
      <div className="kanban-cartao-meta">
        <span>{evento.produto ? evento.produto.referencia : (evento.categoria || '—')}</span>
        <span style={{ color: evento.atrasado ? 'var(--danger)' : undefined }}>
...
function Coluna({ coluna, eventos, onClickCartao }) {
  ...
  return (
    <div ref={setNodeRef} className={`kanban-coluna${isOver ? ' sobre-drop' : ''}`}>
      ...
      {eventos.map((e) => <Cartao key={e.id} evento={e} onClick={onClickCartao} />)}
    </div>
  );
}
...
export default function CalendarioKanban({ eventos, onMudarStatus, onClickCartao }) {
